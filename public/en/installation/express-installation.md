...
![GeoDin® Installer Welcome Screen](<.gitbook/assets/1_Screen (1).jpg>)

## 1. Licence agreement

Please read the licence agreement carefully and proceed by accepting it.

![Licence agreement](<.gitbook/assets/2_Screen (1).jpg>)
...
![FInish installation](.gitbook/assets/6_Screen.jpg)
...
GeoDin can be installed in two modes, depending on your organization's needs: <!-- src: transcript/deployment-infrastructure#installation-modes-desktop -->
...
The `GeoDin.ini` configuration file is placed in the GeoDin installation directory. It controls system paths, licensing behavior, and interface options. <!-- src: help/13/3297 -->

### Path variables


For all paths in the configuration file, the variable `$%GeoDinROOT$` can be used as a placeholder for the GeoDin installation directory. This offers two advantages: <!-- src: help/13/8866 -->
...
When a `ProgramData` entry is defined (e.g., `ProgramData=C:\ProgramData\`), the `$%GeoDinROOT$` variable is redirected to that directory. This allows GeoDin system files to be saved without administrator rights (Windows Vista and later). Without a `ProgramData` entry, `$%GeoDinROOT$` returns the directory above `GeoDin.exe`. <!-- src: help/13/8866 -->
...
|---|---|---|---|
| `SysPath` | Yes | `...\GeoDin\SYSLIB\` | Directory for GeoDin system files | <!-- src: help/13/8866 -->
...
When updating GeoDin, always install the new version in a **different directory** from your previous version. After installation, start GeoDin and apply changes to object types, dictionaries, and measurement value structures from your previous version by navigating to `<System> > <System configuration>` and double-clicking the method icon. <!-- src: help/13/3295 -->

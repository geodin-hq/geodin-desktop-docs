---
description: Reference for object operations beyond installation. Compiled from Help-files content; needs editorial review and possible distribution into per-feature pages.
---
...
Normally however you will want to continue in the **\"data management\"** mask using one of the five editors: General data, Layer data, Sample data, Well design and Data sequences. Detailed information is available in the following chapters.
 ...
 Once you are in the data management mask, there is no need to change over to the GeoDin object manager in order to create another objects. Instead just click the button **New object**. This may be repeated as often as you like. <!-- src: help/03/143 -->

### Data management


When you create a new object the **\"Data management\"** method automatically opens (as described in the previous section). When you want to edit an existing object, select it in the GeoDin object manager and double click the ![Data management](.gitbook/assets/icons/data-management.png) [Data management](../objects/data-management.md) method icon.
...
Please also see the help notes [Using the data entry grid](../measurement-values/working-with-measurement-data.md). <!-- src: help/03/147 -->
...
Layouts are available for SEP1 and SEP3 object types, as well as for geotechnical exploration EN ISO, which can be used to create and print a corresponding borehole log. <!-- src: help/03/151 -->
...
If the object contains either measurement or document data, it will be shown in the dialogue window (black where present; gray if absent). After confirming your decision you can delete the object permanently. <!-- src: help/03/184 -->
...
***WARNING:*** *THIS METHOD CANNOT BE UNDONE!* <!-- src: help/03/188 -->
...
With the button **Apply** the result of the search (for example the code) can be inserted in the current entry field (from which the search was started) at the actual cursor position. To end the search dialogue without applying the result use the button **Cancel** or the **ESC** key <!-- src: help/03/332 -->
...
Please also see the help notes [Using the data entry grid](../measurement-values/working-with-measurement-data.md). <!-- src: help/03/412 -->
...
[Data management](../objects/data-management.md) <!-- src: help/03/2099 -->
...
*The fill pattern table INTENG.SGN (Source INTENG.SGA) contains the English version of the BS5930 fill patterns and symbols and is therefore identical with the fill pattern table GEODDEF.SGN (Source GEODDEF.SGA). The graphical representation of boreholes uses the INTENG fill pattern table for English language objects. Hence changes to the GEODDEF fill pattern table do not affect the display of boreholes. When filling polygons and other areas in the layout editor, the signature table depends on the selection in the corresponding fill dialogue.*\
*For special object types (for example \"Geotechnical borehole\" or \"Botswana Dep. of Geol. Survey\") there are custom made fill pattern files (GEOTECH1 and BOTSWANA).* <!-- src: help/10/2378 -->
...
S_old S_new Plaintext for Key

The \"S_old\" key can now be used when entering data, but it is automatically replaced by \"S_new\" when the entry is saved in the database. All keys that do not have an entry in the DB key field are written directly to the database.

Please note that \"S_new\" must also exist as a separate dictionary entry or be newly created in order to avoid the error \"Abbreviation could not be found in dictionary\" during or after updating the data record.
...
1.  Syntax control and translation control

<!-- -->

1.  Use of the key for graphical representation with signatures

<!-- -->

1.  Assignment of the key to specific key groups
...
The *percentage* value controls the display in the slice head profile. A layer with the specification 100% has the full width of the drilling column, layers with lower percentage values are displayed correspondingly narrower. The value must be between 0 and 100. <!-- src: help/10/3058 -->
...
***Note:*** *Information on groundwater levels must obviously be depth related, hence the option box -Requires entry in brackets- must be checked so that the data input can be controlled.* <!-- src: help/10/3085 -->
...
**2 -** to the right edge. <!-- src: help/10/3088 -->
...
During the export at least one table with codes and text is created. If fill patterns are used in the dictionaries further tables are created containing the appropriate information. <!-- src: help/10/3187 -->
